...
When the LLM's summary is missing or too short (< 200 chars), `_generate_summary_from_diff()` generates a summary from the git diff using a cheap model. The function now uses `diff_char_budget(model)` to determine the diff character limit, making it model-aware instead of using a hardcoded 12,000 character cap. The prompt includes:
