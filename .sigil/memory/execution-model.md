# Execution Model — Worktree Isolation, Inline PR Publishing, Parallel Execution, Directive Bypass, and Cleanup
...
If the LLM's summary is missing or too short (< 200 chars), Sigil generates a summary from the git diff using `_generate_summary_from_diff()`. This function now uses `diff_char_budget(model)` to determine how much of the diff to include, making it model-aware instead of using a hardcoded 12,000 character limit. The prompt includes:
- An authoritative file list parsed from `diff --git` lines
- The model-aware truncated diff (with truncation notice if applicable)
- Instructions to describe ALL listed files even if hunks are truncated
...
## Directive Item Bypass

Directive items (from user-flagged GitHub issues) are handled specially in the pipeline:

1. **Fetched separately** — `fetch_directive_issues()` does an unbounded scan of open Sigil-labeled issues for comments containing the directive phrase
2. **Converted to FeatureIdea** — via `directive_to_idea()`, with `source_issue` set and `disposition="pr"`
3. **Injected before triaged items** — directive items are placed at the front of the PR pipeline, bypassing the triager stage entirely
4. **Bypass `max_prs_per_run` cap** — directive items are always included in the PR batch. Only non-directive items are subject to the cap. If the cap is exceeded, overflow non-directive items are moved to issues.
