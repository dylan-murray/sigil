...
- `_item_key()` — category+file key extraction
- `_extract_marker_keys()` — HTML comment marker extraction from PR/issue bodies (replaces old `_extract_finding_key()` which parsed titles via regex)
- `dedup_items()` — filters duplicates via title match, marker key match, and Jaccard similarity; passes new items
- `fetch_existing_issues()` — mixed issues/PRs, directive detection, body truncation, max cap
- `_format_pr_body()` — finding vs idea, includes `<!-- sigil-key: ... -->` marker for findings
- `_format_issue_body()` — finding, with downgrade context, idea, includes `<!-- sigil-key: ... -->` marker for findings
...
- **Removed:** `_extract_finding_key()` test (replaced by marker-based dedup)
