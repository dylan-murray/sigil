...
- `test_create_worktree_uses_no_track()` — verifies `--no-track` flag is passed to `git worktree add`
- `test_execute_in_worktree_fallback_when_inner_reason_none()` — verifies `failure_reason` never leaks as `None`
