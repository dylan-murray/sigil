# Pipeline Architecture — 7-Stage Async Agentic Workflow

Sigil runs a 7-stage async pipeline. Each stage is optional and can be skipped via config or CLI flags.
...
The triager is biased toward "pr" for anything shippable in one PR — issues are reserved for work that genuinely needs human design.
...
PR bodies include model attribution (which models generated each idea) and a `<!-- sigil-key: ... -->` marker for deduplication.
