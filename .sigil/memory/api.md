...
class FeatureIdea:
    ...
    source_issue: int | None = None  # GitHub issue number if created from a user directive
```
...
- `acompletion(...)` — async LLM completion
- `get_context_window(model: str) -> int` — returns max input tokens for a model
- `diff_char_budget(model: str, *, reserve_tokens: int = 5000, chars_per_token: int = 3) -> int` — returns model-aware diff character budget. Formula: `max(12_000, (context_window - reserve_tokens) * chars_per_token)`. Floors at 12,000 for unknown models.
- `get_max_output_tokens(model: str) -> int` — returns max output tokens for a model
- `get_usage_snapshot() -> dict` — returns current token usage snapshot
- `reset_trace_task(task_id: str) -> None`
- `set_trace_task(task_id: str) -> None`
...
- `fetch_existing_issues(client, *, max_issues, directive_phrase) -> list[ExistingIssue]` — fetches open Sigil-labeled issues for triager dedup context
- `fetch_directive_issues(client, *, directive_phrase) -> list[ExistingIssue]` — unbounded scan of open Sigil-labeled issues whose comments contain the directive phrase
- `directive_to_idea(issue: ExistingIssue) -> FeatureIdea` — wraps a directive issue into a PR-track work item
...
- `generate_pr_summary(item, result, model) -> str` — generates PR summary with model-aware diff budget
